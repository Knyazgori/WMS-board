async function fetchData(DataKey, GeoKey) {
  const data = await fetch(
      `http://localhost:44932/GetInformation/GetInformation?DataKey=${DataKey || "WSS"}&GeoKey=${GeoKey || "WSS"
      }`,
      {
          method: "GET",
          headers: {
...
function renderTable(rows) {
  ...
  function renderRow(item, key) {
          ...
          for (let i = 0; i < key; i++) {
              row += `
                <tr align="center">
                    ...
                    <td>${item[i].Amount || ""}</td>
                    <td><span>${item[i].LotFrom || ""}</span><hr /><span>${item[i].LotTo || ""
                  }</span></td>
                    <td>${item[i].BigNum || ""}</td>
                </tr>`;
          }
      ...
      <tr align="center">
          ...
          <td>${item.PrdDesc || ""}</td>
          <td>${item.Amount || ""}</td>
          <td><span>${item.LotFrom || ""}</span><hr /><span>${item.LotTo || ""
              }</span></td>
          <td>${item.BigNum || ""}</td>
      </tr>`;
      }
  ...
  table += renderHeader();
  table += "</thead><tbody>";
  if (Boolean(rows.Data.Tasks[0].Task?.length)) {
...
setInterval(
  function () {
      fetchData(queryObj.DataKey, queryObj.GeoKey).then((res) => {
